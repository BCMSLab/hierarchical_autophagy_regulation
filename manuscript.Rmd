...
plot_grid(
    ...
    geom_point() +
    geom_line() +
    annotate('text', c(6.5, 6), y = c(0, 1.5),
...
png(filename = 'manuscript/figures/autophagy_categories.png',
...
# figure
png(filename = 'manuscript/figures/model_factors.png',
    height = 22, width = 16, units = 'cm', res = 300)
...
png(filename = 'manuscript/figures/peaks_overlap_enrichment.png',
...
```{r binding_enrichment_states}
mats[c('0', '48', '168')] %>%
    map_df(melt, .id = 'type') %>%
    as_tibble() %>%
    mutate(value = round(value, 1)) %>%
    filter(grepl('Enhancer|Promoter', Var1),
           grepl('PPARG|CEBPB', Var2)) %>%
    dcast(Var2 + type ~ Var1, value.var = 'value')
```
...
pd %>%
    ...
    print(floating = FALSE,
          ...
          comment = FALSE,
          file = 'manuscript/tables/expression_data.tex')
```
...
pd %>%
    ...
    print(floating = FALSE,
          ...
          sanitize.text.function = identity,
          comment = FALSE,
          file = 'manuscript/tables/binding_data.tex')
```
...
pd %>%
    ...
    print(floating = FALSE,
          ...
          sanitize.text.function = identity,
          comment = FALSE,
          file = 'manuscript/tables/hm_data.tex')
```
...
png(filename = 'manuscript/figures/factors_course.png',
    height = 8, width = 16, units = 'cm', res = 300)
grid.newpage()
...
draw(
  targets %>%
    ...
    Heatmap(show_row_names = TRUE,
            ...
            show_row_dend = FALSE,
            show_heatmap_legend = FALSE,
            row_title = 'Gene Targets at (hr)',
            row_title_side = 'right',
            column_title = 'Time (hr)',
            column_title_side = 'bottom'),
  newpage = FALSE,
  padding = unit(c(0, 7, 7, 0), "mm")
)
